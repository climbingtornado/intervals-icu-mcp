...
### Added
- Calendar event tags are now readable and writable. The `Event` model already parsed `tags` from the API, but every tool dropped them before responding, so tags set in the Intervals.icu web UI never reached the model and there was no way to set them from a conversation. `icu_create_event` and `icu_update_event` take an optional `tags` list, `icu_bulk_create_events` documents `tags` among its per-event fields, and `tags` now appears in the responses of those three tools plus `icu_get_calendar_events`, `icu_get_upcoming_workouts`, and `icu_get_event` (omitted when empty, like other empty fields). On update the list *replaces* the existing tags — live-verified, not merged — so the description tells the model to include any tags it wants to keep, and `[]` clears them; leaving `tags` out keeps them untouched. The API stores a leading `#` literally (`"#indoor"` stays `"#indoor"`), so the descriptions tell the model to leave it off rather than the server silently stripping it. Contributed by @russellmays (#136, closes #16).
...
### Fixed
- `icu_apply_sport_settings` told the model to run it "after changing FTP/FTHR/pace to backfill chart math", and claimed it recomputes training load. Neither is true: it overwrites the zones on *every* past activity of the sport — there is no date range — while FTP and other thresholds stored on those activities are kept. Following the old advice after a new threshold or lab test would rewrite the athlete's whole history with zones that did not exist yet. The description now says what it does and that it is not needed after `icu_update_sport_settings`, whose values apply from now on while past activities keep the settings they were analysed with. For selected activities or a date range it now points the user to Update zones in the web UI's activity list view, which the public API does not offer — live-checked: `/api/v1/activity/{id}/analyze` returns 404 and the web app's route rejects API keys. The `icu_delete_sport_settings` description and docs no longer claim that deleting settings rewrites historical chart math, for the same reason. Behaviour confirmed against the Intervals.icu web UI's apply dialog (#138, #140, refs #137).
...
- `icu_bulk_create_workouts` and `icu_create_workout_folder` close the loop on authoring a plan in one conversation. Building a 12-week plan previously meant one `icu_create_workout` call per session into a folder the athlete had to create in the web UI first; `icu_create_workout_folder` now creates the `FOLDER` or `PLAN` and returns the `id` to pass straight back as `folder_id`, and `icu_bulk_create_workouts` fills it from a single JSON array whose items are shaped exactly like an `icu_create_workout` call. Bulk items get the validation the singular tool gets from its signature — `folder_id` and `name` required, `day` an integer — reported per item as `Workout 3: ...` so a model can fix the one bad entry rather than re-sending the array blind. Raw Intervals.icu field names in a bulk payload are rejected with an error naming the replacement (`moving_time -> duration_seconds`) rather than passed through, the rule `icu_bulk_create_events` has enforced since 5.0.0, and `event_type` is accepted as an alias for `workout_type` because models reuse the event vocabulary. Neither tool touches the calendar: dated entries still go through `icu_bulk_create_events`, and a finished plan is scheduled with `icu_apply_training_plan`. Contributed by @russellmays (#133).

These five additions take the server from 62 tools to 67 — `safe` (the default) from 59 to 64, `none` from 56 to 60.

### Fixed
- `icu_get_upcoming_workouts` returns calendar *events*, not workout-library templates, and nothing in its description said so. The name was merely ambiguous while the library was read-only; with `icu_update_workout` and `icu_delete_workout` now registered, it became a live failure mode — a model that fetched "upcoming workouts" and passed one of those `id`s to a library tool would be addressing a different object entirely. The description now states that each `id` is a calendar event ID belonging to `icu_get_event` / `icu_update_event` / `icu_delete_event`, and points at `icu_get_workouts_in_folder` for reusable templates (#132).
- `icu_create_workout` and `icu_update_workout` reported metrics the workout does not have. The API returns `0.0` rather than null for unset numeric fields — live-verified on a Ride with no distance — so a created workout came back carrying `distance_meters: 0.0`, `training_load: 0`, and `intensity_factor: 0.0`, which reads as a measured zero rather than an absent value. Zero metrics are now omitted, matching what `icu_get_workouts_in_folder` already did (#132).
